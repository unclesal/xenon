...
//    typedef boost::geometry::model::segment<boost_location_t> boost_segment_t;
    ...
    // Одна точка маршрута.
    struct waypoint_t {
        // Номер по порядку. Формируется в агенте при построении
        // полетного плана. Позволяет идентифицировать точку,
        // удалять точки вплоть до данной при достижении и т.д.
        uint16_t npp = 0;
    ...
    struct aircraft_parameters_t {
        ...
        float length = 6.0;
        // Центр поворота. Зависит от длины и модели самолета.
        float rotation_center = 6.0 / 2.0;
